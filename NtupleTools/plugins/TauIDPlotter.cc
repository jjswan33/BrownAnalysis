#include "BrownAnalysis/NtupleTools/plugins/TauIDPlotter.h"
...
TauIDPlotter::TauIDPlotter(const edm::ParameterSet& iConfig):
  ...
  srcWeight_(iConfig.getParameter<edm::InputTag>("srcWeight")),
...
TauIDPlotter::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup)
{
   ...
   if(iEvent.getByLabel(srcWeight_,weightHandle))
     weight =*weightHandle;
   ...
   if(iEvent.getByLabel(ref_,ref)&&ref->size()>0) {
	 ...
	 for(edm::View<Candidate>::const_iterator j = ref->begin();j!=ref->end();++j)
	   if(j->pt()>ptThres_)
	     {
	       rawSummary->Fill(0.);
	       histos[0]->Fill(j->pt(),weight);
	       histosEta[0]->Fill(j->eta(),weight);
	       histosV[0]->Fill(vertices);

	       if(j==ref->begin()) {
		 histosL[0]->Fill(j->pt(),weight);
		 rawSummaryL->Fill(0.);
		 ...
		 for(pat::TauCollection::const_iterator i=src->begin();i!=src->end();++i) 
		   if(ROOT::Math::VectorUtil::DeltaR(i->p4(),j->p4()) < matchDR_&&i->pt()>ptNum_) {
		     ...
		     if(passed){
		       histos[k+2]->Fill(j->pt(),weight);
		       histosEta[k+2]->Fill(j->eta(),weight);
